...
def parse_git_command(text):
    ...
    # --- Commit all (default) ---
    # --- Commit all with variations ---
    if "commit all" in text or "commit changes" in text or text.strip() == "commit":
        return ["git add -A", 'git commit -m "voice commit-all"']

# --- Commit + push ---
    if "commit and push" in text or "commit push" in text:
        return ["git add -A", 'git commit -m "voice commit-commit and push"', "git push"]

    # --- Push ---
    if "push" in text:
    # Auto commit before push if there are staged/unstaged changes
        return ["git add -A", 'git commit -m "voice commit-Push"', "git push"]


    # --- Pull ---
    if "pull" in text:
        return ["git add -A", 'git commit -m "voice commit-Pull"', "git pull"]
